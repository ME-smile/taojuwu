...
class SalesAnalysisLineChart extends StatefulWidget {
  final int type;
  final List<SaleStatisticsDataModel> models;
  SalesAnalysisLineChart({Key key, this.models, this.type}) : super(key: key);

  @override
  _SalesAnalysisLineChartState createState() => _SalesAnalysisLineChartState();
}

class _SalesAnalysisLineChartState extends State<SalesAnalysisLineChart> {
  List<SaleStatisticsDataModel> get models => widget.models;
  ...
  LineDataSet initDataset() {
    for (int i = 0; i < models?.length ?? 0; i++) {
      SaleStatisticsDataModel model = models[i];
      entries.add(
